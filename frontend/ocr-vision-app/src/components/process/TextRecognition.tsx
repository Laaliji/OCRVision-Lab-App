...
import { faUpload, faSpinner, faTrash, faMagic, faRedo } from '@fortawesome/free-solid-svg-icons';
import { OCRService } from '../../services/api';
import { WordRecognitionResult } from '../../types/types';
import { useLanguage } from '../../contexts/LanguageContext';

type RecognitionMode = 'character' | 'text';

const TextRecognition: React.FC = () => {
  const { t } = useLanguage();
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<string | null>(null);
  const [recognitionMode, setRecognitionMode] = useState<RecognitionMode>('character');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await OCRService.processWordRecognition(file, recognitionMode);
      setResult(result);
    } catch (err) {
      ...
      {!result ? (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <form onSubmit={handleSubmit}>
            {/* Mode Selection */}
            <div className="mb-6">
              <h2 className="text-lg font-medium text-gray-800 mb-3">{t('textRecognition.modeSelection')}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div
                  className={`border ${
                    recognitionMode === 'character' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200'
                  } rounded-lg p-4 cursor-pointer hover:border-indigo-300 transition`}
                  onClick={() => handleModeChange('character')}
                >
                  <div className="flex items-center">
                    <div className="w-6 h-6 rounded-full border-2 border-indigo-500 flex items-center justify-center mr-3">
                      {recognitionMode === 'character' && (
                        <div className="w-3 h-3 rounded-full bg-indigo-500"></div>
                      )}
                    </div>
                    <div>
                      <h3 className="font-medium text-gray-800">{t('textRecognition.characterMode')}</h3>
                      <p className="text-sm text-gray-500">{t('textRecognition.characterModeDesc')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div
                  className={`border ${
                    recognitionMode === 'text' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200'
                  } rounded-lg p-4 cursor-pointer hover:border-indigo-300 transition`}
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="font-medium text-gray-800">{t('textRecognition.textMode')}</h3>
                      <p className="text-sm text-gray-500">{t('textRecognition.textModeDesc')}</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <tr className="bg-indigo-50">
                    <td className="py-2 px-4 font-medium text-indigo-800">
                      {result.recognition_mode === 'text'
                        ...
                        : t('textRecognition.characterByCharacter')}
                    </td>
